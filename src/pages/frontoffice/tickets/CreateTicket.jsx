...
export default function CreateTicket() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true); // Active le chargement pendant la création
            const ticket = await createItem("Ticket", ticketPayload);
            if (ticket) {
                for (const elId of selectedElements) {
                    const el = elements.find(item => item.id === elId);
                    if (el) {
                        await createItem("Item_Ticket", {
            ...
            setConfirmMessage("Erreur lors de la création du ticket.");
            setConfirmType("danger");
            setShowConfirm(true);
        } finally {
            setLoading(false); // Désactive le chargement à la fin
        }
    };
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="row g-3 mb-4 p-3 rounded-3" style={{ backgroundColor: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.05)" }}>
                            <div className="col-sm-6 col-md-3">
                                ...
                                <select className="form-select border-secondary-subtle shadow-none" value={type} onChange={e => setType(e.target.value)} required disabled={loading}>
                            ...
                            <div className="col-sm-6 col-md-3">
                                ...
                                <select className="form-select border-secondary-subtle shadow-none" value={status} onChange={e => setStatus(e.target.value)} required disabled={loading}>
                            ...
                            <div className="col-sm-6 col-md-3">
                                ...
                                <select className="form-select border-secondary-subtle shadow-none" value={priority} onChange={e => setPriority(e.target.value)} required disabled={loading}>
